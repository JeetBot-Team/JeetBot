...
module.exports = async (msg, args, store) => {
  if (msg.member.hasPermission([`MANAGE_MESSAGES`])) {
    ...
    if (guildInfo.ListenInfo.channel_ID) {
      destination = await msg.guild.channels.cache.find(
        (ch) => ch.id === guildInfo.ListenInfo.channel_ID
      );
      msg.channel.send(
        `${msg.author}, I'm listening now to ${msg.channel}.\nIf you want to change where I'm sending messages to, type j.channel #channel`
      );
    } else if (!guildInfo.ListenInfo.channel_ID) {
      msg.channel.send(
        `${msg.author}, which channel do you want send listened messages to?\nPlease enter j.channel #channel.`
      );
    }
    ...
    collector.on(`collect`, async (m, col) => {
      ...
      if (
        ...
        m.content.startsWith(`j.channel`) &&
        !isNaN((m.content.indexOf(`#`) + 1, m.content.length - 1))
      ) {
        let listen_channelID = m.content.slice(
          m.content.indexOf(`#`) + 1,
          m.content.length - 1
        );
        let listen_channelName = m.content.slice(
          m.content.indexOf(`<`),
          m.content.indexOf(`>`) + 1
        );
        ...
        destination = await msg.guild.channels.cache.find(
          (ch) => ch.id === listen_channelID
        );
      }

      if (destination) {
        if (
          m.content.toLowerCase() === `j.done` &&
          msg.author.id === m.author.id
        ) {
